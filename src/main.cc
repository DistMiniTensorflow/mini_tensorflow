...
void RunWorker() {
  ...
  int mid_dim =
    distlr::ToInt(ps::Environment::Get()->find("MID_DIM"));
  int out_dim =
    distlr::ToInt(ps::Environment::Get()->find("OUT_DIM"));
  ...
  distlr::LR lr = distlr::LR(num_feature_dim, mid_dim, out_dim);
  lr.SetKVWorker(kv);
  ...
  if (rank == 0) {
    ...
    std::cout << "Push init Weights: " << rank << std::endl;
  ...
  for (int i = 0; i < num_iteration; ++i) {
    ...
    if (rank == 0 and (i + 1) % test_interval == 0) {
      ...
      //std::string modelfile = root + "/models/part-00" + std::to_string(i + 1);
